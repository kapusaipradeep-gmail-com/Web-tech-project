...
$sql3="SELECT COUNT(ATTENDANCE) FROM attendances WHERE ATTENDANCE='PRESENT' AND COURSE_ID='".$row[0]."' AND ROLL='".$q."'";
      ...
      $html1=$html1."<td>".$absent."</td>";
			if($present+$absent==0)
			{
				$html1=$html1."<td>".intval(0)."%</td><td><a href='#'onclick=fun2('$q@$row[0]') >View Details </a></td>";
	     $html1=$html1."</tr>";
			}
			else {
				$html1=$html1."<td>".intval(($present*100/($present+$absent)))."%</td><td><a href='#'onclick=fun2('$q@$row[0]') >View Details </a></td>";
	     $html1=$html1."</tr>";
				// code...
			}
